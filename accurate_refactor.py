...
# 1. Replace the single fetch with a comment
...
# 2. Replace the fetch validation
    ...
    if fetched_count < len(watchlist) * 0.70:
        ...
        if not is_test_mode:
            try:
                ...
                pass"""
new_validation = """    if not is_test_mode:
        try:
            upsert_scanner_health(scanner_name="REVERSAL", status="RUNNING", error_msg=None)
        except Exception:
            pass"""
...
end_idx = -1
for i, line in enumerate(lines):
    if line.strip() == "logger.info(" and "rejection summary" in lines[i+1]:
        end_idx = i
        break

if start_idx != -1 and end_idx != -1:
    # Indent the symbol loop body by 4 spaces
    for i in range(start_idx, end_idx):
        if lines[i].strip() != "":
            lines[i] = "    " + lines[i]

    # Insert cleanup block at end_idx
    cleanup_block = [
