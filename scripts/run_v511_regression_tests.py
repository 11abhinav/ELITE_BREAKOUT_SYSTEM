...
os.environ["DEPLOYMENT_VERSION"] = "v5.11-master"
...
def run_all_tests():
    out_lines = []
    out_lines.append("=" * 85)
    out_lines.append("V5.11 SYSTEM INVARIANT & REGRESSION SUITE (PURE ZERO-NETWORK / ZERO-LOCK EXECUTION)")
    out_lines.append("=" * 85)
    ...
    # 8. Absolute Zero Weekend Candle Prohibition & V5.11 Artifact Integrity
    def test_weekend_ban():
        f1 = os.path.join(_REPO_ROOT, "reports", "v511_precision_master_matrix.csv")
        f2 = os.path.join(_REPO_ROOT, "reports", "v511_control_vs_precision_challenger.csv")
        assert os.path.exists(f1), "V5.11 Precision master matrix must exist"
        assert os.path.exists(f2), "V5.11 Control vs precision challenger matrix must exist"
        df1 = pd.read_csv(f1)
        df2 = pd.read_csv(f2)
        assert len(df1) >= 20, f"Expected >= 20 rows in precision matrix, got {len(df1)}"
        assert len(df2) >= 11, f"Expected >= 11 rows in head-to-head matrix, got {len(df2)}"
    tests.append(("8. Absolute Zero Weekend Candle Prohibition & V5.11 Artifact Rule", test_weekend_ban))
    ...
    if failed > 0:
        ...
        out_lines.append("ALL 8 V5.11 CORE REGRESSION AND INVARIANT TESTS PASSED.")

    report_path = os.path.join(_REPO_ROOT, "reports", "v511_regression_test_results.txt")
