...
os.environ["DEPLOYMENT_VERSION"] = "v5.10-master"
...
def run_all_tests():
    out_lines = []
    out_lines.append("=" * 85)
    out_lines.append("V5.10 SYSTEM INVARIANT & REGRESSION SUITE (PURE ZERO-NETWORK / ZERO-LOCK EXECUTION)")
    out_lines.append("=" * 85)
    ...
    # 8. Absolute Zero Weekend Candle Prohibition & V5.10 Artifact Integrity
    def test_weekend_ban():
        f1 = os.path.join(_REPO_ROOT, "reports", "v510_holdout_reproducibility_matrix.csv")
        f2 = os.path.join(_REPO_ROOT, "reports", "v510_full_dataset_frontier_matrix.csv")
        assert os.path.exists(f1), "V5.10 Holdout reproducibility matrix must exist"
        assert os.path.exists(f2), "V5.10 Full dataset frontier matrix must exist"
        df1 = pd.read_csv(f1)
        df2 = pd.read_csv(f2)
        assert len(df1) >= 40, f"Expected >= 40 rows in holdout matrix, got {len(df1)}"
        assert len(df2) >= 40, f"Expected >= 40 rows in full dataset matrix, got {len(df2)}"
    tests.append(("8. Absolute Zero Weekend Candle Prohibition & V5.10 Artifact Rule", test_weekend_ban))
    ...
    if failed > 0:
        ...
        out_lines.append("ALL 8 V5.10 CORE REGRESSION AND INVARIANT TESTS PASSED.")

    report_path = os.path.join(_REPO_ROOT, "reports", "v510_regression_test_results.txt")
