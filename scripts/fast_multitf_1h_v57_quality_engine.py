#!/usr/bin/env python3
# =============================================================================
# scripts/fast_multitf_1h_v57_quality_engine.py
# V5.7 MULTI-TF 1H QUALITY-RECOVERY & FRONTIER STABILIZATION ENGINE
# =============================================================================
# Purpose: Recover PF >= 1.50, E[R] >= +0.25R, and positive CI lower bound
# while preserving sample maturity (N >= 75-100) across 393 hourly parquets.
# Evaluated under DEV -> VAL -> LOCKED HOLDOUT governance.
# =============================================================================
...
def run_multitf_1h_quality():
    print("=" * 110)
    print("V5.7 MULTI-TF 1H QUALITY-RECOVERY ENGINE (PF >= 1.50, E[R] >= +0.25R, N >= 75-100)")
    print("Goal: Fine-tune compression ratio, volume confirmation, and asymmetric target extension")
    print("=" * 110)
    t0 = time.time()
    ...
    # ── Define Multi-TF 1H Quality Challenger Suite ──────────────────────────
    challengers = [
        # Quality Challenger 1: 20H Base, 1.45 ATR, 0.68 Comp, 1.75x Vol, 2.6R Target
        {"id": "M1H_V57_QUAL_A_20H_26R", "base_bars": 20, "max_width_atr": 1.45, "max_comp": 0.68, "min_vol": 1.75, "min_cpos": 0.70, "stop": "SHELF", "t_r": 2.6, "allowed_regimes": ["BULL", "NEUTRAL"], "desc": "Quality A: 20H Base, 1.45 ATR Width, 0.68 Comp, 1.75x Vol, 2.6R Target"},

        # Quality Challenger 2: 20H Base, 1.50 ATR, 0.70 Comp, 1.65x Vol, 2.8R Target (Asymmetric Extension)
        {"id": "M1H_V57_QUAL_B_20H_28R", "base_bars": 20, "max_width_atr": 1.50, "max_comp": 0.70, "min_vol": 1.65, "min_cpos": 0.70, "stop": "SHELF", "t_r": 2.8, "allowed_regimes": ["BULL", "NEUTRAL"], "desc": "Quality B: 20H Base, 1.50 ATR Width, 0.70 Comp, 1.65x Vol, 2.8R Target"},

        # Quality Challenger 3: 22H Base (~3.5 Days), 1.40 ATR, 0.65 Comp, 1.80x Vol, 2.5R Target
        {"id": "M1H_V57_QUAL_C_22H_PINCH", "base_bars": 22, "max_width_atr": 1.40, "max_comp": 0.65, "min_vol": 1.80, "min_cpos": 0.72, "stop": "SHELF", "t_r": 2.5, "allowed_regimes": ["BULL", "NEUTRAL"], "desc": "Quality C: 22H Base, 1.40 ATR Width, 0.65 Deep Pinch, 1.80x Vol, 2.5R Target"},

        # Quality Challenger 4: 18H Base, 1.40 ATR, 0.68 Comp, 1.70x Vol, 2.7R Target
        {"id": "M1H_V57_QUAL_D_18H_27R", "base_bars": 18, "max_width_atr": 1.40, "max_comp": 0.68, "min_vol": 1.70, "min_cpos": 0.70, "stop": "SHELF", "t_r": 2.7, "allowed_regimes": ["BULL", "NEUTRAL"], "desc": "Quality D: 18H Base, 1.40 ATR Width, 0.68 Comp, 1.70x Vol, 2.7R Target"},

        # Quality Challenger 5: 20H Base, 1.45 ATR, 0.70 Comp, 1.60x Vol, 3.0R Target (Runner Extension)
        {"id": "M1H_V57_QUAL_E_20H_30R", "base_bars": 20, "max_width_atr": 1.45, "max_comp": 0.70, "min_vol": 1.60, "min_cpos": 0.70, "stop": "SHELF", "t_r": 3.0, "allowed_regimes": ["BULL", "NEUTRAL"], "desc": "Quality E: 20H Base, 1.45 ATR Width, 0.70 Comp, 1.60x Vol, 3.0R Target"},

        # Quality Challenger 6: 20H Base, Multi-Regime with Regime-Adapted Filtering
        {"id": "M1H_V57_QUAL_F_MULTI_REG", "base_bars": 20, "max_width_atr": 1.45, "max_comp": 0.70, "min_vol": 1.70, "min_cpos": 0.70, "stop": "SHELF", "t_r": 2.6, "allowed_regimes": ["BULL", "NEUTRAL", "BEAR"], "desc": "Quality F Multi-Regime: 20H Base, 2.6R Target across Bull+Neut+Bear"},
    ]
    ...
    for h_path in h1_files:
        ...
        for i in range(25, n_h - 15):
            ...
            candle_range = hi - li
            cpos = (ci - li) / candle_range if candle_range > 0 else 0.5
            ...
            for spec in all_specs:
                ...
                # 1. Breakout above prior base high
                base_h = np.max(h_h[i - base_bars : i])
                base_l = np.min(h_l[i - base_bars : i])
                if ci <= base_h:
                    continue

                base_width_atr = (base_h - base_l) / daily_atr if daily_atr > 0 else 9.99
                if base_width_atr > max_width:
                    continue

                # 2. Compression ratio (First half vs Second half volatility)
                half_b = base_bars // 2
                ...
                # 3. Hourly Volume Thrust & Close Position
                if vol_ratio < min_vol:
                    continue
                if cpos < min_cpos:
                    continue

                # 4. Stop Loss: Anchor below base low (shelf)
                ...
                # 5. Forward Outcome Evaluation over forward daily bars (up to 20 days)
    ...
    out_csv = os.path.join(_REPORTS_DIR, "multitf_1h_v57_quality_outcomes.csv")
    ...
    for spec in all_specs:
        ...
        for part in ["DEV", "VAL", "HOLDOUT"]:
            ...
            highlight = "🟢" if (part == "HOLDOUT" and er >= 0.25 and pf >= 1.50 and n_trades >= 70) else "  "
