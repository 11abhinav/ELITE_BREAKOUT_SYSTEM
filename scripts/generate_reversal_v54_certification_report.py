#!/usr/bin/env python3
# =============================================================================
# scripts/generate_reversal_v54_certification_report.py
# Institutional Analysis & Formal Promotion Report for Reversal V5.4
# =============================================================================
...
_OUTPUT_REPORT = os.path.join(_REPO_ROOT, "reports", "reversal_v54_certification_report.md")
...
def compute_metrics(df_subset):
    ...
    if n == 0:
        return {
            ...
            "avg_bars": 0.0, "bear_er": "N/A", "neut_er": "N/A", "bull_er": "N/A",
...
def main():
    ...
    print(f"Loaded {len(df):,} records from {_CSV_PATH}")

    # Unique variants
    variants = df["variant_id"].unique()

    print("\n" + "="*120)
    print(f"{'VARIANT ID':<32} | {'PARTITION':<8} | {'N':>5} | {'WIN%':>6} | {'E[R]':>7} | {'PF':>5} | {'95% CI':>14} | {'MAX DD':>7} | {'BEAR':>9} | {'NEUT':>9} | {'BULL':>9}")
    print("="*120)
    ...
    for v in variants:
        ...
        for part in ["DEV", "VAL", "HOLDOUT"]:
            ...
            m = compute_metrics(p_df)
            ci_str = f"[{m['ci_low']:+.2f}, {m['ci_high']:+.2f}]"
            print(f"{v:<32} | {part:<8} | {m['n']:>5} | {m['win_pct']:>5.1f}% | {m['er']:>+6.3f}R | {m['pf']:>5.2f} | {ci_str:>14} | {m['max_dd_r']:>6.1f}R | {m['bear_er']:>9} | {m['neut_er']:>9} | {m['bull_er']:>9}")
            summary_rows.append({
                "variant_id": v,
                "partition": part,
                **m
    ...
    sum_df.to_csv(os.path.join(_REPO_ROOT, "reports", "reversal_v54_summary_matrix.csv"), index=False)
    print(f"\nSummary matrix saved to reports/reversal_v54_summary_matrix.csv")
