#!/usr/bin/env python3
# =============================================================================
# scripts/generate_short_covering_v56_certification_report.py
# V5.6 Short Covering Specialist Institutional Report Generator
# =============================================================================
...
_CSV_PATH = os.path.join(_REPO_ROOT, "reports", "short_covering_v56_specialist_outcomes.csv")
...
def compute_sc_metrics(df_subset):
    n = len(df_subset)
    if n == 0:
        return {
...
def main():
    ...
    print("\n" + "="*125)
    print(f"{'VARIANT ID':<34} | {'PART':<7} | {'N':>4} | {'WIN%':>6} | {'E[R]':>7} | {'PF':>5} | {'95% CI':>14} | {'MAX DD':>7} | {'BEAR':>8} | {'NEUT':>8} | {'BULL':>8}")
    print("="*125)
    ...
    for v in variants:
        ...
        for part in ["DEV", "VAL", "HOLDOUT"]:
            ...
            m = compute_sc_metrics(p_df)
            ci_str = f"[{m['ci_low']:+.2f}, {m['ci_high']:+.2f}]"
            print(f"{v:<34} | {part:<7} | {m['n']:>4} | {m['win_pct']:>5.1f}% | {m['er']:>+6.3f}R | {m['pf']:>5.2f} | {ci_str:>14} | {m['max_dd_r']:>6.1f}R | {m['bear_er']:>8} | {m['neut_er']:>8} | {m['bull_er']:>8}")
            summary_rows.append({
                "variant_id": v,
                "partition": part,
                **m
    ...
    sum_df.to_csv(os.path.join(_REPO_ROOT, "reports", "short_covering_v56_summary_matrix.csv"), index=False)
    print(f"\nSummary matrix saved to reports/short_covering_v56_summary_matrix.csv")
