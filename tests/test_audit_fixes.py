...
class TestAuditFixes(unittest.TestCase):
    ...
    @patch('pullback_pipeline.get_recent_alerts_for_scanner', return_value=set())
    ...
    def test_pullback_health_telemetry_counts(self, mock_regime, mock_ret, mock_fetch, mock_wl, mock_health, mock_recent, mock_init_db):
    ...
    def test_upstox_1h_resample_returns_valid_normalized_data(self):
        ...
        with patch('config.UPSTOX_ACCESS_TOKEN', 'mock_token'), \
            ...
            self.assertEqual(res_1h.symbol, "NEULANDLAB")
            self.assertEqual(res_1h.timeframe, "1h")
            self.assertFalse(res_1h.dataframe.empty)
            self.assertIsNotNone(res_1h.provenance)
