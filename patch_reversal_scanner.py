...
with open("app/reversal_scanner.py", "r") as f:
    content = f.read()
...
old_check_missing = """                if symbol not in all_ticker_data or all_ticker_data[symbol] is None:
                    ...
                    logger.debug(f"[REVERSAL] {symbol} rejected: No data available")
                    continue
                ...
                if isinstance(all_ticker_data[symbol], ProviderResult):
                    ...
                    if res == ProviderResult.RATE_LIMIT:
                        rejection_reasons["no_data"] += 1
                        logger.debug(f"[REVERSAL] {symbol} rejected: Rate limited")
                    else:
                        rejection_reasons["no_data"] += 1
                        logger.debug(f"[REVERSAL] {symbol} rejected: Data fetch error ({res})")
                    continue
...
new_check_missing = """                if symbol not in all_ticker_data or all_ticker_data[symbol] is None:
                ...
                if hasattr(md, 'dataframe'):
                    ...
                    if df is None:
                        rejection_reasons["no_data"] += 1
                        logger.debug(f"[REVERSAL] {symbol} rejected: No valid MarketData (error={md.error})")
                        continue
                else:
                    if isinstance(md, ProviderResult):
                        rejection_reasons["no_data"] += 1
                        logger.debug(f"[REVERSAL] {symbol} rejected: Data fetch error ({md})")
                        continue
                    df = md
                    
                if df is None or (hasattr(df, 'empty') and df.empty):
                    rejection_reasons["no_data"] += 1
                    logger.debug(f"[REVERSAL] {symbol} rejected: Empty DataFrame")
                    continue
                    
                ticker = df.copy()"""

content = content.replace(old_check_missing, new_check_missing)
with open("app/reversal_scanner.py", "w") as f:
    f.write(content)
print("Patched reversal_scanner.py")
