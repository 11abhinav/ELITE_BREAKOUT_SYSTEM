...
_BUILT_IN_VARIANTS: List[Dict[str, Any]] = [
    ...
    # -----------------------------------------------------------------------
    # 11. TECHNICAL SCANNER (Frozen Top 4 Proven Patterns Only)
    # -----------------------------------------------------------------------
    dict(variant_id="TECH_PROD_WYCKOFF_SPRING_TYPE_2", scanner_family=ScannerFamily.TECHNICAL,
         description="Production Champion: Wyckoff Spring Type 2 (Structural Alpha, King of Volatility)",
         parameters={"pattern": "WYCKOFF_SPRING_TYPE_2", "min_score": 70, "rvol_gate": 1.20},
         status=VariantStatus.CHAMPION),
    dict(variant_id="TECH_PROD_BULL_FLAG", scanner_family=ScannerFamily.TECHNICAL,
         description="Production Champion: Bull Flag Continuation (100% Regime Invariant)",
         parameters={"pattern": "BULL_FLAG", "min_score": 70, "rvol_gate": 1.20},
         status=VariantStatus.CHAMPION),
    dict(variant_id="TECH_PROD_MULTI_MONTH_BASE", scanner_family=ScannerFamily.TECHNICAL,
         description="Production Champion: Multi-Month Base Breakout (Low Drawdown, Bull Dominance)",
         parameters={"pattern": "MULTI_MONTH_BASE_BREAKOUT", "min_score": 70, "rvol_gate": 1.20},
         status=VariantStatus.CHAMPION),
    dict(variant_id="TECH_PROD_UNDERCUT_AND_RALLY", scanner_family=ScannerFamily.TECHNICAL,
         description="Production Champion: Undercut & Rally (Structural Reclaim Core)",
         parameters={"pattern": "UNDERCUT_AND_RALLY", "min_score": 70, "rvol_gate": 1.20},
         status=VariantStatus.CHAMPION),
    dict(variant_id="TECH_RES_FLAT_BASE_BREAKOUT", scanner_family=ScannerFamily.TECHNICAL,
         description="Research Only: Flat Base Breakout (Gated in bear/chop; retained for research)",
         parameters={"pattern": "FLAT_BASE_BREAKOUT", "status": "RESEARCH_ONLY"},
         status=VariantStatus.RETIRED),
    dict(variant_id="TECH_RES_ASCENDING_TRIANGLE", scanner_family=ScannerFamily.TECHNICAL,
         description="Research Only: Ascending Triangle (Rangebound specialist; retained for research)",
         parameters={"pattern": "ASCENDING_TRIANGLE", "status": "RESEARCH_ONLY"},
         status=VariantStatus.RETIRED),
    dict(variant_id="TECH_QUAR_CUP_AND_HANDLE", scanner_family=ScannerFamily.TECHNICAL,
         description="Quarantined: Cup & Handle (Breakeven alpha + 84.1R DD; quarantined)",
         parameters={"pattern": "CUP_AND_HANDLE", "status": "QUARANTINED"},
         status=VariantStatus.RETIRED),
    dict(variant_id="TECH_QUAR_VCP_CONTRACTION", scanner_family=ScannerFamily.TECHNICAL,
         description="Quarantined: Standalone VCP Contraction (High false breakout rate; quarantined)",
         parameters={"pattern": "VCP_CONTRACTION", "status": "QUARANTINED"},
         status=VariantStatus.RETIRED),
    dict(variant_id="TECH_QUAR_HIGH_TIGHT_FLAG", scanner_family=ScannerFamily.TECHNICAL,
