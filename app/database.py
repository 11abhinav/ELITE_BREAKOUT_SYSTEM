...
def save_multibagger_alert(symbol: str, alert_price: float, breakout_type: str = None, 
                        fm_score: float = None, notes: str = None,
                        position_pct: float = None, position_amount: float = None,
                        position_shares: int = None,
                        portfolio_bucket: str = None, valuation_score: float = None,
    ...
    try:
        stale_indicators = ("CACHED_PREV_DAY", "CACHED_MULTI_DAY", "MISSING_PARTIAL")
        if data_quality and str(data_quality).upper() in stale_indicators:
            logger.warning(f"🛡️ save_multibagger_alert: Suppressing BUY for {symbol} due to data_quality={data_quality}")
            return False

        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute('''
                    INSERT INTO multibagger_alerts (
                        symbol, alert_price, breakout_type, fm_score,
                        notes, position_pct, position_amount, position_shares,
                        portfolio_bucket, valuation_score,
                        momentum_score, momentum_confidence, data_quality, fallback_timestamp,
                        alert_date, alert_time, current_price, current_score
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT ON CONSTRAINT uq_multibagger_symbol_date DO NOTHING
                ''', (
                    symbol, alert_price, breakout_type or 'MULTIBAGGER', fm_score,
                    notes, position_pct, position_amount, position_shares,
                    portfolio_bucket, valuation_score, momentum_score, momentum_confidence,
                    data_quality, fallback_timestamp,
                    ist_today, ist_time, alert_price, fm_score
                ))
                inserted = cur.rowcount > 0
                conn.commit()
                if inserted:
                    logger.info(f"✅ Saved Multibagger BUY alert for {symbol} at ₹{alert_price}")
                return inserted
    except Exception as e:
        logger.exception(f"❌ Failed to save multibagger alert")
        return False

def _get_multibagger_positions(is_closed: bool = None, symbol: str = None, trade_date: str = None, days_back: int = None) -> list:
    try:
        with get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                query = "SELECT * FROM multibagger_alerts WHERE 1=1"
                params = []
...
def get_multibagger_alerts(symbol: str = None, days_back: int = 30) -> list:
    return _get_multibagger_positions(is_closed=False, symbol=symbol, days_back=days_back)

def get_multibagger_closed_positions(symbol: str = None, days_back: int = 30) -> list:
    return _get_multibagger_positions(is_closed=True, symbol=symbol, days_back=days_back)

def update_multibagger_real_time_prices(symbols_metrics: dict) -> int:
    with _DB_WRITE_LOCK:
        updated_count = 0
        try:
            with get_connection() as conn:
                ...
                try:
                    with conn.cursor() as cur:
                        for symbol, metrics in symbols_metrics.items():
                            ...
                            if symbol and price is not None and price > 0:
                                cur.execute('''
                                    UPDATE multibagger_alerts 
                                    SET current_price = %s, current_score = COALESCE(%s, current_score), status_updated_at = NOW()
                                    WHERE symbol = %s AND is_closed = FALSE
                                ''', (price, score, symbol))
                                updated_count += cur.rowcount
                        conn.commit()
...
def close_multibagger_position(symbol: str, exit_price: float, exit_signal: str = None, force_close: bool = False) -> bool:
    with _DB_WRITE_LOCK:
        try:
            with get_connection() as conn:
                success = False
                try:
                    with conn.cursor() as cur:
                        cur.execute('''
                            SELECT id, alert_price FROM multibagger_alerts 
                            WHERE symbol = %s AND is_closed = FALSE
                            ORDER BY alert_date DESC, alert_time DESC
                            LIMIT 1
                        ''', (symbol,))
                        ...
                        ''', (exit_price, exit_date, exit_time, exit_signal, pnl_rs, pnl_pct, alert_id))
                        
                        conn.commit()
                        success = True
                        
                        if cur.rowcount > 0:
                            logger.info(f"🔒 Multibagger Position Closed: {symbol} at ₹{exit_price} | P&L: {pnl_pct:.2f}%")
                            # Add notification
                            p_str = f"₹{pnl_rs:.2f}" if pnl_rs is not None else f"{pnl_pct:.2f}%"
                            msg = f"{symbol} | Exit: ₹{exit_price:.2f} | P&L: {p_str}"
                            insert_notification('sell', f'Multibagger Exit', msg, symbol)
                            return True
                        return False
                finally:
                    if not success:
            ...
            logger.exception(f"❌ Failed to close multibagger position for {symbol}")
            return False
