    ...
    with _INIT_LOCK:
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                # Adds the idempotency_key column and its unique partial index to the alerts table
                # to prevent duplicate alerts generated by candidate confirmations under retries.
                cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS idempotency_key VARCHAR(200)")
                ...
                # 4.5. [MIGRATION]: Alert Quality Improvement Database Schema - Wave 1
                try:
                    cur.execute("SAVEPOINT alert_quality_wave1_migration;")

                    # Create scanner_evaluation_log table
                    cur.execute("""
                    ...
                    # Alter alerts table to add Wave 1 tracking columns
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS evaluation_id TEXT;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS scanner_run_id TEXT;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS mfe_pct REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS mae_pct REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS mfe_r REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS mae_r REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS gross_realized_r REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS net_realized_r REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS entry_timestamp TIMESTAMPTZ;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS exit_timestamp TIMESTAMPTZ;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS t1_timestamp TIMESTAMPTZ;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS t2_timestamp TIMESTAMPTZ;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS t3_timestamp TIMESTAMPTZ;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS sl_timestamp TIMESTAMPTZ;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS entry_bar_id INTEGER;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS exit_bar_id INTEGER;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS time_to_entry REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS time_to_t1 REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS time_to_t2 REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS time_to_sl REAL;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS bars_to_t1 INTEGER;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS bars_to_sl INTEGER;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS outcome_labels JSONB DEFAULT '{}'::jsonb;")
                    cur.execute("ALTER TABLE alerts ADD COLUMN IF NOT EXISTS weighted_realized_r REAL;")

                    cur.execute("RELEASE SAVEPOINT alert_quality_wave1_migration;")
                    logger.info("✅ Database schema migration successfully verified for Alert Quality Improvement Wave 1")
                except Exception as ex:
                    cur.execute("ROLLBACK TO SAVEPOINT alert_quality_wave1_migration;")
                    logger.error(f"❌ Failed to run Alert Quality Improvement schema migration: {ex}")
