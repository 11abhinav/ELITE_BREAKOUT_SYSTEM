...
def run_financial_strength_engine(symbol: str, raw_data: Dict[str, Any]) -> EngineResult:
    ...
    if is_financial:
        ...
        if raw_data.get('capital_adequacy_ratio') is None: missing.append("capital_adequacy_ratio")
        ...
        if raw_data.get('debt_equity') is None: missing.append("debt_equity")
    ...
    if is_financial:
        ...
        if raw_data.get('gross_npa') is None: missing.append("gross_npa")
        ...
        if raw_data.get('interest_coverage_ratio') is None: missing.append("interest_coverage_ratio")
    ...
    if is_financial:
        ...
        if raw_data.get('net_npa') is None: missing.append("net_npa")
        ...
        if debt_eq == 0.0:
            ...
            if raw_data.get('debt_yoy_growth') is None: missing.append("debt_yoy_growth")
    ...
    if is_financial:
        ...
        if raw_data.get('altman_z') is None: missing.append("altman_z")
    ...
    if is_financial:
        ...
        if raw_data.get('liquidity_coverage_ratio') is None: missing.append("liquidity_coverage_ratio")
        ...
        if raw_data.get('current_ratio') is None: missing.append("current_ratio")
