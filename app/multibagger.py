...
def run_exit_monitor(price_data_map: dict, cache: dict, is_test_mode: bool = False):
    ...
    # Exit Monitor must NEVER evaluate on Saturday or Sunday to prevent false exits.
    now_ist = datetime.now(IST)
    if now_ist.weekday() >= 5:
        logger.info(f"🚫 [EXIT MONITOR] Today is {now_ist.strftime('%A')} (Weekend). Exit Monitor execution strictly prohibited on weekends.")
        return
