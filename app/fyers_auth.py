...
def save_access_token(auth_code: str) -> str:
    """Exchanges auth_code for access_token and saves it locally."""
    try:
        session = get_session_model()
        ...
        logger.info(f"Fyers access token updated and saved to {token_path}")
        return access_token
    except Exception as e:
...
def get_access_token() -> str:
    """Retrieves the access token from the local cache file."""
    token_path = config.FYERS_TOKEN_PATH
