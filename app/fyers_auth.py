...
def auto_login() -> Optional[str]:
    ...
    with _auto_login_lock:
        ...
        try:
            ...
            logger.info("🔑 [VERSION: FYERS_AUTH_v44.0_SYMBOL_FORMAT_GATE_VALIDATED_STABLE] Starting Fyers headless OAuth auto-login flow...")
            ...
            logger.info(f"Fyers login Step 5: Exchanging/Validating credential token (len={len(auth_code)}) for all-day access token...")
            try:
                real_token = None
                for test_id in (target_app_id, successful_app_id):
                    session_m = get_session_model(client_id=test_id)
                    session_m.set_token(auth_code)
                    response = session_m.generate_token()
                    if response and isinstance(response, dict) and "access_token" in response:
                        real_token = response["access_token"]
                        logger.info(f"✅ Fyers Step 5: generate_token() SUCCESS for '{test_id}' | AccessToken len={len(real_token)} | Saving to DB & local cache.")
                        return save_access_token_direct(real_token)
                    else:
                        logger.warning(f"Fyers Step 5 generate_token() failed for '{test_id}': {response}")
                
                logger.info(f"Fyers Step 5: generate_token() returned failures. Testing token directly against live Fyers API profile...")
                try:
                    from fyers_apiv3 import fyersModel
...
_autologin_in_progress = False
_last_autologin_start_time = 0.0
...
def get_access_token() -> str:
    ...
    global _cached_token, _token_date, _autologin_attempted_date, _last_autologin_fail_time, _last_fyers_err_log_time, _autologin_in_progress, _last_autologin_start_time
    ...
    with _token_lock:
        ...
        # 3. Suppress repeat auto-login attempts if already running or within cooldown window
        if _autologin_in_progress:
            return None

        if (_autologin_attempted_date == now_date and (now_ts - _last_autologin_fail_time < 900.0)) or (now_ts - _last_autologin_start_time < 120.0):
            if now_ts - _last_fyers_err_log_time > 300.0:
                _last_fyers_err_log_time = now_ts
                logger.warning(f"⚠️ [FYERS AUTH] Fyers token missing for today ({now_date}). Auto-login in cooldown. Please authenticate via /fyers/login.")
            return None

        # 4. Attempt auto-login in background thread (strictly single-threaded)
        _autologin_in_progress = True
        _last_autologin_start_time = now_ts
        ...
        def _bg_auto_login():
            global _last_autologin_fail_time, _autologin_in_progress
            try:
                token = auto_login()
                if token:
                    ...
                    logger.warning(f"⚠️ [FYERS AUTH] Auto-login failed — please authenticate via /fyers/login.")
            except Exception as _bg_err:
                _last_autologin_fail_time = time.time()
                logger.warning(f"⚠️ [FYERS AUTH] Background auto-login encountered exception: {_bg_err}")
            finally:
                _autologin_in_progress = False
