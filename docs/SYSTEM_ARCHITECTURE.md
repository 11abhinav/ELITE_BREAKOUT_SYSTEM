...
| :--- | :--- | :--- | :--- | :--- |
| **2026-07-27**<br>`PENDING` | `SCANNER_HEALTH_STATUS_PAUSED_FIX_v1.0` | `scanner_health` PostgreSQL table DDL check constraint `chk_scanner_status` excluded `'PAUSED'` and `'STOPPED'`, causing Admin Pause/Stop actions to fail with `psycopg2.errors.CheckViolation`. | Updated `chk_scanner_status` constraint in `app/database.py` to explicitly include `'PAUSED'` and `'STOPPED'`. Added dynamic SQL migration in `init_db()` to drop and recreate constraint on boot. | Eliminates `CheckViolation` exceptions during admin scanner pause/stop actions and ensures 100% reliable state persistence in PostgreSQL `scanner_health`. |
