...
| :--- | :--- | :--- | :--- | :--- |
| **2026-07-26**<br>`PENDING` | `CANONICAL_EVALUATOR_PARITY_v1.0` | `stock_analyzer.py` independently recreated simplified scanner rules for EOD, Reversal, Pullback, Wealth, and Multibagger, causing subtle logic drift and parity discrepancies from production background scanners. | Exposed `evaluate_<scanner>_symbol(symbol, df, fund_data, regime_ctx)` in all production scanner modules (`eod_scanner.py`, `reversal_scanner.py`, `pullback_pipeline.py`, `wealth_engine.py`, `multibagger.py`) and called them directly in `stock_analyzer.py`. | Guarantees 100% production scanner logic parity for single-stock diagnostics and manual alert promotion without side effects. |
