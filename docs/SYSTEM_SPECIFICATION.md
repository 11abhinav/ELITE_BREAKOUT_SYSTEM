  ...
  - **100% Parity Across All 7 Scanner Stages**:
    ...
    - *Stage 2 (EOD Breakout)*: Validates 20D high breakout, volume surge ($\ge 1.8\text{x}$ 20D median), upper wick ($\le 35\%$), body ratio ($\ge 45\%$), close position ($\ge 65\%$), ATR expansion ($\ge 0.9$), EMA20 extension ($\le 1.5$ ATR), and bullish candle.
    - *Stage 3 (Multi-TF Intraday)*: Evaluates 15-minute volume explosion spikes during live market hours (09:30–14:45 IST); explicitly labeled `INTRADAY ONLY` during off-market runs.
    - *Stage 4 (Reversal Oversold)*: Evaluates mean-reversion oversold bounce ($20\%-45\%$ 52W High drop band, $\text{RSI}(14) \le 38$ or RSI curl $\ge 50$, $\text{Close} > \text{SMA}_{50}$ reclaim).
  ...
  - **Deep Analysis & Strict Qualification Gate Enforcement for Alert Creation**: Manual alert creation (`🚀 Raise Alert`) is strictly gated to run ONLY after full Deep Analysis is executed (`is_deep_analysis = True`) and enforces scanner allowlist validation (`scanner in ALLOWED_SCANNERS`) and stage qualification (`CORE MET` / `QUALIFIED`). Calculates exact Stop Loss and Target levels ($T_1, T_2, T_3, T_4$) using actual 20-day ATR (`bundle.atr_20.iloc[-1]`), rejecting unqualified or invalid scanner requests with HTTP 400.
