  ...
  - **100% Parity Across All 7 Scanner Stages**:
    - *Stage 1 (Daily Builder)*: Validates price floor ($\ge ₹100.0$), turnover ($\ge ₹1.0\text{ Cr}$), and bar history ($\ge 50$).
    - *Stage 2 (EOD Breakout)*: Validates 20D high breakout, volume surge ($\ge 1.8\text{x}$ 20D median), upper wick ($\le 35\%$), and bullish candle.
    - *Stage 3 (Multi-TF Intraday)*: Evaluates 15-minute volume explosion spikes during live market hours (09:30–14:45 IST).
    - *Stage 4 (Reversal Oversold)*: Evaluates mean-reversion oversold bounce ($\text{RSI}(14) \le 35.0$).
    - *Stage 5 (Pullback Continuation)*: Executes full `swing_utils` swing pivot detection (`detect_confirmed_pivots`), impulse origin selection (`select_pullback_origin`), retracement depth & volume contraction measurement (`measure_pullback`), and resumption trigger bar validation (`detect_resumption_trigger`), guaranteeing 100% parity with `pullback_pipeline.py`.
    - *Stage 6 (Wealth Engine)*: Evaluates all 4 fundamental buckets matching `wealth_engine.py` 100% (Core Compounder: $\text{ROCE} \ge 20\%$, $\text{ROE} \ge 15\%$, $\text{D/E} \le 0.50$; Growth Multiplier: YoY Revenue $\ge 20\%$, YoY Profit $\ge 20\%$; Quality-On-Sale: $\text{ROCE} \ge 15\%$, $\text{D/E} \le 1.0$, Drop from 52W High $\ge 15\%$; Opportunistic: YoY Profit $\ge 40\%$) coupled with mandatory CMP > SMA200 technical trend gate.
    - *Stage 7 (Multibagger Engine)*: Uses `safe_val()` and `safe_row()` in `compute_piotroski()` to handle multi-row Pandas DataFrame/Series indexing cleanly without Pandas ambiguity exceptions, evaluating Piotroski F-Scores (e.g. `8/9`) and promoter pledge ($\le 10\%$).
  - **Inline Main Screen Diagnostic Layout**: Renders 7-stage scanner diagnostics directly inline on the main screen (`#stock-diagnostic-main-container`) right below the search widget rather than in an overflowing modal popup, with smooth auto-scroll into view and a 1-click collapse button (`✕ Close Diagnostic View`).
  ...
  - **Repositioned Advanced Outcome Analytics (PREVIEW)**: Placed directly below the Scanner Health KPI grid (`#kpis`) for logical workflow flow.
  ...
  - **Strict Per-User Watchlist Isolation (`user_watchlists`)**: Safely handles integer vs string user IDs (e.g. `DEFAULT_USER`, `admin`, `57880`) across session queries and database functions using strict per-user filtering (`WHERE user_id::text = %s`), guaranteeing 100% data privacy and isolation so each user only views and manages their own personal watchlist.
