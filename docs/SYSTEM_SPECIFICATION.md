...
- **4-Stage Timeframe Cascade**:
  1. **Phase A (1H Trend Filter)**: Evaluates 3-month Hourly data (~437 bars). Mandates $\text{EMA}_9 > \text{EMA}_{20} > \text{EMA}_{50}$, $\text{Close} > \text{EMA}_{200}$ (or reduced trend gate for 50-199 bar symbols), and $\text{ADX}_{14} \ge 18$.
...
- **Key Eligibility & Quality Gates**:
  1. **Drop Band Gate**: Stock must be **20.0% to 45.0% below its 52-week high**.
  2. **Trend Structure Reclaim**: $\text{Close} \ge \text{SMA}_{50}$ (or within 3% holding $\text{EMA}_{20}$).
  3. **Oversold RSI Curl**: $\text{RSI} \le 45$, curling upward with current value $\ge 50$.
...
3. **Profit Booking & Exit Profiles (`EXIT_PROFILES`)**:
   ...
   - **Target 4 (`target_4`) Status**: `target_4` is an **informational structural runner target** used for analytical quality scoring and dashboard display. Position size is 100% liquidated by T3.
