  ...
  - **Safe Session Validity Type-Casting (`check_session_validity`)**: Safely handles integer vs string user IDs (e.g. `DEFAULT_USER`, `admin`) in session validation queries (`users` table `user_id` column integer casting), preventing SQL syntax errors and avoiding unexpected session wipes / abrupt logouts during watchlist operations.
  ...
  - **7-Stage Scanner Funnel Table & CORE MET Orange Badge**: Fast-path evaluation through all 6 scanners. Stages meeting `CORE MET` render in a high-visibility **ORANGE / Amber badge (`#f59e0b`)** with bold uppercase guidance (`⚡ CORE CRITERIA MET! ADD TO WATCHLIST TO GET FINAL STATUS & deep pipeline evaluation`). Stocks are offloaded to personal watchlist for asynchronous deep pipeline processing.
