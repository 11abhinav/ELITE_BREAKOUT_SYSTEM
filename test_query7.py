...
from database import get_scanner_execution_history, get_scanner_health_status
...
for name in ["EOD", "REVERSAL", "MULTI_TF", "MULTIBAGGER", "PULLBACK"]:
    health = get_scanner_health_status(name)
    if health:
        print(health.get('scanner_name'), health.get('date'), health.get('provider_stats'))
